...
async def export_compliance_report(session_id: str, format: str = "json", _: bool = Depends(require_engine_ready)):
    ...
    if format == "json":
        ...
        # Generate a minimal HTML-based PDF-style report (text/html, printable)
        html_rows = ""
        ...
        writer.writerow(["ARGUS Compliance Report"])
        writer.writerow(["Session", session_id])
        writer.writerow(["Generated", report["timestamp"]])
        writer.writerow([])
        writer.writerow(["Metric", "Value"])
        writer.writerow(["Actions Evaluated", report["actions_evaluated"]])
        writer.writerow(["Actions Blocked", report["actions_blocked"]])
        writer.writerow(["Actions Quarantined", report["actions_quarantined"]])
        writer.writerow(["Approved", report["actions_approved"]])
        writer.writerow(["Denied", report["actions_denied"]])
        if audit_log:
            writer.writerow([])
